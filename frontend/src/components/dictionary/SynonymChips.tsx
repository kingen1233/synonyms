...
export function SynonymChips({
    ...
    return (
        <>
            ...
            {/* Direct synonyms */}
            {direct.length > 0 && (
                <Stack direction="row" spacing={1} useFlexGap sx={{ flexWrap: 'wrap', mb: transitive.length > 0 ? 1.5 : 0 }}>
                    {direct.map((synonym) => (
                        <Chip
                            key={synonym}
                            label={synonym}
                ...
                <>
                    <Stack direction="row" spacing={1} useFlexGap sx={{ flexWrap: 'wrap' }}>
                        {transitive.map((transitive) => (
                            <Tooltip
                                key={transitive.word}
                                title={`Linked via "${transitive.closestNeighbour}"`}
                            >
                                <Chip
                                    icon={<LinkIcon />}
                                    label={transitive.word}
                                    size={chipSize}
                                    sx={{
                                        fontWeight: 600,
                                        bgcolor: '#fffbeb',
